...
class BaseTrainer:
    ...
    def _do_train(self, rank=-1, world_size=1):
        ...
        nw = round(self.args.warmup_epochs * nb)  # number of warmup iterations
        last_opt_step = -1
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for i, batch in pbar:
                ...
                if ni <= nw and nw > 0:
                    ...
                    self.accumulate = max(1, round(self.args.nbs / self.batch_size))
                    for j, x in enumerate(self.optimizer.param_groups):
                        x['lr'] = x['initial_lr'] * self.lf(epoch)
                        if 'momentum' in x:
                            x['momentum'] = self.args.momentum
                ...
                if rank in {-1, 0}:
                    pbar.set_description(
                        ...
                        (f'{epoch + 1}/{self.epochs}', mem, *losses, batch['cls'].shape[0], batch['img'].shape[-1], self.optimizer.param_groups[0]['lr']))
